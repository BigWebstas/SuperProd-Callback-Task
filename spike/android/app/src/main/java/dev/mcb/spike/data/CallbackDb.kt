 ...
 *  - [QueuedTask] is the outbound retry queue. One row per call that needs to
 *    reach the plugin. PENDING rows are served at /pending; an ack moves the row
 *    to SENT; the retry pass ages stale rows to FAILED after MAX_ATTEMPTS.
 *
...
data class QueuedTask(
    ...
    val callLogId: Long,
    val payload: String,          // JSON the plugin consumes
...
interface QueueDao {
    ...
    @Query("SELECT * FROM queued_task ORDER BY createdAt DESC")
    fun all(): List<QueuedTask>

    @Query("SELECT * FROM queued_task WHERE id = :id")
    fun byId(id: Long): QueuedTask?

    @Query("UPDATE queued_task SET state = :state, lastError = :err, updatedAt = :ts WHERE id = :id")
    fun setState(id: Long, state: String, err: String?, ts: Long)
    ...
    companion object {
        ...
        fun get(context: Context): CallbackDb = instance ?: synchronized(this) {
            instance ?: Room.databaseBuilder(
                context.applicationContext, CallbackDb::class.java, "callback-spike.db"
